...
void JPetGeantParser::processMCEvent(JPetGeantEventPack* evPack)
{
  ...
  for ( uint i = 0; i < evPack->GetNumberOfHits(); i++) {
    ...
    std::cout << evPack->GetHit(i)->GetEvtID() << " " << evPack->GetHit(i)->GetTrackID() << std::endl;
